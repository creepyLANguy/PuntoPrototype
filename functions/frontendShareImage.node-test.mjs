...
test("share export uses 1080x1350 as both CSS and PNG dimensions", () => {
  ...
  assert.match(source, /const highResolutionCanvas = await toCanvas/);
  ...
  assert.match(source, /highResolutionCanvas\.width/);
  assert.match(source, /highResolutionCanvas\.height/);
  assert.match(source, /'image\/png'/);
  assert.doesNotMatch(source, /toBlob\(clone/);

...
test("a failed direct-link join sound is ignored and never replayed on a later gesture", async () => {
  ...
  await settle(250);
  assert.equal(
    window.__audioTestState.starts,
    0,
    "a blocked direct-link join sound must never replay after later gestures",
  );
});

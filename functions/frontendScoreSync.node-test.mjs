...
test("renders every consecutive score snapshot immediately", () => {
  ...
  assert.equal(
    applyActiveScoreSnapshot(firstScore, listenerToken, listenerToken, renderScore),
    true,
  );
  assert.equal(
    applyActiveScoreSnapshot(secondScore, listenerToken, listenerToken, renderScore),
    true,
  );
...
test("ignores snapshots from an inactive court listener", () => {
  ...
  assert.equal(
    applyActiveScoreSnapshot({}, 3, 4, () => renderCount++),
    false,
  );
